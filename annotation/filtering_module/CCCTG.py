...
def EFF(effects):
    ...
    for effect in effects:
        if "HIGH" in effect or "MODERATE" in effect and "protein_coding" in effect or 1 == 1:
...
def ANN(effects):
    snp_dictionary={}
    for effect in effects:
        if "HIGH" in effect or "MODERATE" in effect or 1 == 1:
    ...
    if not "#" == line[0]:
        ...
        for gene in snp_dictionary:
            for variant in snp_dictionary[gene]:
                ...
                variant_list.append([chrom,pos,id_,ref, alt,feature ,variant,gene,zygosity,cadd,popfreq,exac])
...
i=0;
header=["Chromosome","Position","ID","Ref","Alt","amino-acid","effect","Gene","zygosity","CADD","thousand_genome","exac"]
j=0
for item in header:
    ws0.write(i, j, item)
    j+=1
